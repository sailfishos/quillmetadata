...
bool QuillMetadata::initialized = false;
QHash<QuillMetadata::Tag,ExifTag> QuillMetadata::m_exifTags;
QHash<QuillMetadata::Tag,XmpTag> QuillMetadata::m_xmpTags;
...
bool QuillMetadata::isValid()
{
    return ((m_exifData != 0) || (m_xmpPtr != 0));
}

QVariant QuillMetadata::entry(QuillMetadata::Tag tag)
{
...
QVariant QuillMetadata::entryExif(QuillMetadata::Tag tag)
{
    if (!m_exifData)
        return QVariant();
...
QVariant QuillMetadata::entryXmp(QuillMetadata::Tag tag)
{
    if (!m_xmpPtr)
        return QVariant();
...
bool QuillMetadata::writeXmp(const QString &fileName)
{
    if (!m_xmpPtr)
        return true;
...
QByteArray QuillMetadata::dumpExif()
{
    if (!m_exifData)
        return QByteArray();
